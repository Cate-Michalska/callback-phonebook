...
def searchData(search_info):
    ...
    if search_info.isdigit(): 
        ...
        # if BITHDAY is an ilogical value or incorrect lenght, an error message prints
            if (int(search_info[-4:-1]+search_info[-1])) > 2022 or int(search_info[0:2]) > 12 or int(search_info[2:4]) > 31:
                ...
                cur.execute("SELECT FIRST_NAME, LAST_NAME FROM usertable WHERE BIRTHDATE = '"+search_info+"'")
        # if PHONENUMBER is an incorrect lenght, an error message prints
        elif len(search_info) == 10:
            cur.execute("SELECT FIRST_NAME, LAST_NAME FROM usertable WHERE PHONENUMBER = '"+search_info+"'")
        else:
            print("**ERROR: Please enter a valid Phone Number, Birthday, or First and/or Last Name.")
    ...
    # if var search_info is not a digit, it is sorted into FIRST_NAME or LAST_NAME statement        
        ...
        cur.execute("SELECT FIRST_NAME, LAST_NAME FROM usertable WHERE FIRST_NAME = '"+search_info+"'") or cur.execute("SELECT FIRST_NAME, LAST_NAME FROM usertable WHERE LAST_NAME = '"+search_info+"'")
